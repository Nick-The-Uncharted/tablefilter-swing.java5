 ...
 * different order.
 *
 * <p/>TableSorter registers itself as a listener to the underlying model, just
 * as the JTable itself would. Events recieved from the model are examined,
 ...
 * sorter will resort the rows the next time a value is requested.</p>
 *
 * <p>When the tableHeader property is set, either by using the setTableHeader()
 ...
 * In addition, a mouse listener is installed with the following behavior:</p>
 *
 * <ul>
 *   <li>Mouse-click: Clears the sorting status of all other columns and
 *     advances the sorting status of that column through three values:
 *     {NOT_SORTED, ASCENDING, DESCENDING} (then back to NOT_SORTED again).</li>
 *   <li>SHIFT-mouse-click: Clears the sorting status of all other columns and
 *     cycles the sorting status of the column through the same three values, in
 *     the opposite order: {NOT_SORTED, DESCENDING, ASCENDING}.</li>
 ...
 *     already sorting - giving a way to initiate a compound sort.</li>
 * </ul>
 *
 * <p/>This is a long overdue rewrite of a class of the same name that first
 * appeared in the swing table demos in 1997.</p>
 *
 * <p>Some VERY MINOR changes (coderazzi) to reflect Java6 behavior and avoid
 * warnings</p>
 *
 * @author   Philip Milne
